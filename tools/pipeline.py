...
class Pipeline:
  ...
  def track(video_source, tracking_method, detection_method, benchmark=False):
    ...
    video_source (string): path to the video
    tracking_method (string): the method of tracking
    detection_method (string): the method to dectect the initial frame
    benchmark (boolean): if set, the method will measure the performance of the tracking. The video is not shown. 
    ...
    while True:
      ...
      if not benchmark:
        cv2.imwrite(f"outputs/tracking/{count}.jpg", frame)
  ...
  def detect(images_path, method, benchmark=False):
    ...
    for image in images:
      ...
      if not benchmark:
        cv2.imwrite(f"outputs/detection/detection.jpg", image)
